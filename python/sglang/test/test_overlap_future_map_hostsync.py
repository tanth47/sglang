...
@unittest.skipUnless(torch.cuda.is_available(), "requires SGLang GPU backend imports")
class TestHostSyncSupportContracts(unittest.TestCase):
    def test_hybrid_backend_delegates_cpu_length_capability(self):
        from sglang.srt.layers.attention.hybrid_attn_backend import HybridAttnBackend

        def make(prefill, decode):
            ...
            return HybridAttnBackend(
                runner,
                SimpleNamespace(needs_cpu_seq_lens=prefill),
                SimpleNamespace(needs_cpu_seq_lens=decode),
            )

        self.assertFalse(make(False, False).needs_cpu_seq_lens)
        self.assertTrue(make(True, False).needs_cpu_seq_lens)
        self.assertTrue(make(False, True).needs_cpu_seq_lens)

    def test_filter_batch_uses_host_keep_indices(self):
        from sglang.srt.speculative.dflash_info_v2 import DFlashDraftInputV2

        info = DFlashDraftInputV2.create_idle_input(device=torch.device("cpu"))
        info.reserved_seq_lens_cpu = torch.tensor([10, 20, 30, 40], dtype=torch.int32)
        info.reserved_seq_lens_sum = 100
        info.future_indices = torch.tensor([5, 6, 7, 8])

        info.filter_batch(
            new_indices=torch.tensor([0, 2]),
            has_been_filtered=False,
            new_indices_cpu=[0, 2],
        )

        self.assertTrue(torch.equal(info.reserved_seq_lens_cpu, torch.tensor([10, 30])))
        self.assertEqual(info.reserved_seq_lens_sum, 40)
        self.assertTrue(torch.equal(info.future_indices, torch.tensor([5, 7])))
